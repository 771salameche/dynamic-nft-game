...
task("deploy:loot-box", "Deploys the LootBox contract").setAction(async (_, hre) => {
  ...
  deployments[network.name].LootBox = address;
  fs.writeFileSync(
    deploymentsPath,
    JSON.stringify(deployments, null, 2)
  );

  console.log("LootBox address saved to deployments/deployments.json");
});
